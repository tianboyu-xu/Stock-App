...
- **Home workspace tri-view** - Home has History / Watchlist / Today tabs, with History as the default view; a watchlist row opens its confirmed latest report with mouse or keyboard, and its notice always follows the current lookup-in-progress, lookup-failed, or confirmed-no-detail state; every stock-bar request and the data refresh after task completion enter the unsettled state immediately, so stale stock-bar and fallback reports stay unavailable while status is being reconfirmed or is unknown; Refresh retries both the watchlist and detail status, with per-stock fallback lookups using a fixed concurrency bound and obsolete batches cancelled on refresh or page-state changes; Watchlist supports batch submission for all stocks or only those not analyzed today; watchlist rows can be reordered by dragging, and the new order is persisted to `STOCK_LIST`
- **Current-position summary** - The Home summary includes a plus action for recording a stock, purchase date, purchase price, quantity, and account (Robinhood/RSU/401K/HSA); position rows support editing the stock code and share quantity via a dedicated shares column, can be reordered by dragging the stock cell like the watchlist, and show total price, same-account allocation next to the stock column, and the latest trigger date/price with buy/sell side kept even for stale triggers. Account % is calculated within each account type so stocks in the same account sum to 100%. A profit chart below the table supports 30/60/90/180/365-day windows with month/year ticks for long ranges and calculates different purchase lots separately before combining them. It reads the current quote and calculates total gain/loss amount, return percentage, and after-tax CAGR, with stock-code colors following summary buy/sell triggers and missing watchlist symbols fetched separately. Entries are stored in browser `localStorage`; Robinhood/RSU use an estimated 35% tax before one year and 15% after one year, while 401K/HSA use 0% tax and losses are not taxed. Each row also has a sell button that asks for the sell date, sell price, and sell quantity and automatically calculates the profit as (sell price - purchase price) x quantity; partial sales are supported, sale records are stored in browser local storage with a realized-profit total, and the profit chart shows a dashed cumulative realized-profit line on a second right-hand axis. A collapsible trade history table below the chart lists all sale records with edit (profit recalculated), delete (record only), and revert (sold quantity added back to the original lot) actions
