...
- [新功能] Web/API 启动时自动检查已配置的本机 Ollama，未运行时后台启动 `ollama serve`；远程地址不自动启动，检查失败不阻塞服务启动，可用 `OLLAMA_AUTO_START=false` 关闭。
- [新功能] When the Web/API starts, automatically check the configured local Ollama service and launch `ollama serve` in the background when needed; remote endpoints are not started locally, startup failures do not block the server, and `OLLAMA_AUTO_START=false` disables it.

- [修复] 分析任务因 Ollama 连接失败时不再只显示截断的通用错误，任务消息直接给出启动 Ollama 的指引（`ollama serve`，详见 FAQ Q12c），完整 hint 保留在任务 error 字段中。

- [修复] 触发阈值输入改为失焦/回车提交（微调按钮短防抖），输入过程不再逐字重载导致整页移位；首页摘要在阈值触发的后台刷新期间保留旧条目并显示刷新指示，不再塌陷为加载态。

- [修复] MATR v2 将现金退避依据改为相对现金的预期优势，扩大技术候选并按历史窗口分段验证；与其他 Trigger Strategy 同图对比 NAV，修正 SPY 跨季度收益及当前季度已完成窗口遗漏，新增现金占比、持仓及底层策略归因诊断，保留旧版冻结记录。
- [新功能] Auto Tune 新增 MATR 季度宏观策略路由：使用 ALFRED 历史版本、季度嵌套向前验证与现金退避选择 A–F，季度内冻结参考策略，按 10 个交易日窗口最多买卖各一次，并展示宏观阶段估计、相关性与选型后悔值。
- [修复] Auto Tune v7 修正 DMI/ADX 的 Wilder 平滑与预热，低交易数候选改用有界证据扣分保留真实收益/回撤排序，爬山已评估候选完整参与训练 Top-K；保持原 UI 与测试隔离，旧结果需重新运行。
- [新功能] 首页摘要每只股票直链实时价格图并在条目内展示最近触发（买入/卖出、日期、价格、3天内/7天内/7天外/近7天无触发）；点击摘要即打开对应实时价格图核对 7 天 / 3 天综合评分触发，有分析详情时同步打开。
- [改进] 首页当前持仓标题本身可点击展开/折叠（与右侧折叠按钮同逻辑），折叠按钮样式保持与其他面板一致。
- [改进] 首页摘要股票条目样式恢复为仅展示代码（保留点击直链实时价格图与悬停查看最新触发）。
- [改进] 触发阈值区每行放更多阈值（2/3/4/6 列改为 3/4/6/8 列），减少占用行数。
- [改进] 价格图与 NAV 图更紧凑并收进同一边框面板：价格图高 340→260、NAV 图高 260→200，NAV 卡片去独立边框，读作一张图。
- [新功能] 价格图关联当前持仓：持有该股票时在收盘线标出每个买入点（买入日起首根 K 线处的双环圆点标记，详情在悬停提示中），图例同步增加持仓买入项。
- [修复] Do not display the automatic backtest segment-end liquidation as a SELL trigger for every strategy.

- [改进] 触发阈值区横向收紧：网格列间距减半，标签与输入框间距减半，输入框收窄到 w-14。

- [改进] 触发阈值区进一步压高：卡片内边距减半，输入框去纵向内边距，网格放宽到 6 列，恢复默认值按钮缩到 h-5。

- [改进] 触发阈值区取消折叠常开并移到时间范围下方，标题与小号恢复默认值按钮同行，31 个阈值并为单股对齐网格。

- [改进] BOL 常数与 MACD 买卖阈值步进改为 0.1；持仓锚定切换不再搬动 NAV 卡片位置。

- [改进] 触发阈值区改单行紧凑布局并合并为一股输入流，高度大幅压缩。

- [改进] 价格图标题行合一：标题、现价、综合评分、股票同行，图例常显且占满整行；触发开关单行横滑；日期刻度加密到 7 个（窄屏仍自动稀疏）。

- [改进] 触发阈值设置默认展开。

- [改进] 价格图标题行合一：标题、现价、图例开关、股票同行展示，图例面板移至下方全宽；价格图取消折叠常开。

- [修复] 触发图例默认收起并在窄屏占满整行，展开不再挤压图表；窄图日期刻度自动稀疏到 3 个，共享中轴不再重叠。

- [改进] 触发图例收进标题右侧可折叠的图例开关，默认收起；堆叠态两图共用一条中间日期轴（实时价格日历），NAV 自带 X 轴自动隐藏；持仓锚定下价格图策略标记与明细统一按买入日过滤。

- [改进] 价格图与 NAV 图连为一张图：触发图例移到标题右侧、综合评分移到价格图上方，NAV 说明与图例移到 NAV 图下方，上下无缝衔接。

- [改进] 价格图与 NAV 图连为一体：触发图例移到价格图上方，NAV 标题与图例移到 NAV 图下方，两图日期轴在中间相接。

- [改进] 技术指标价格图触发开关按股票记住，下次打开同只股票自动恢复。
- [改进] Adaptive Budget / NAV 卡片移到价格图正下方，两图绘图区对齐、日期轴在中间相接；持仓锚定开启时保持独立完整视图。

- [改进] 已成交交易明细按日期排序并展示成交价，卖出附相对最近买入的盈亏（绿色盈利/红色亏损，不含费用）；已成交交易在所选策略 NAV 线上打点；Strategy G 默认买入/卖出分数加入 6/-6（默认 6,7,8 / -6,-7,-8，前后端默认同步）。
- [修复] 技术指标价格图触发标记去重：方向统一表达买卖（买朝上/卖朝下），OBV 改用菱形，复合信号改用空心三角，BOLL/CCI/MFI 卖出改用空心图形，图例同步；已成交交易明细方向文字固定买绿卖红并按方向分组，持仓锚定开启时明细同步过滤到持仓期。

- [新功能] Adaptive Budget / NAV 图新增持仓锚定开关：当前股票在首页当前持仓中时自动出现，开启后五条策略线与参考线均自最早买入日起按 1.0 起算，便于对照持仓成本比较策略。
- [修复] Adaptive Budget / NAV 图同时展示基础、趋势、多因子、自适应预算、ACES 五条策略线，当前选中策略加粗高亮，其余策略明细与交易标记仍按所选策略展示；五条线改用蓝/绿/橙/紫/品红区分，S&P 500 改为灰色虚线、CAGR 目标改为黄色点线作为参考线；自适应预算与 ACES 改用首个可用策略曲线兜底，避免选择字段过期时整条线消失，无曲线策略在图下明确列出原因。
- [改进] 技术指标价格图恢复指标触发开关（BUY/SELL、MACD、OBV、KDJ、RSI、BOLL、CCI、DMI、MFI），与策略触发下拉共存：开关控制实时阈值触发标记，下拉控制回测策略买卖标记。
- [改进] Auto Tune 始终包含 ACES 且移除关闭选项；默认预设改为 Aggressive，BUY 阈值默认 7,8、SELL 阈值默认 -7,-8。
- [改进] Auto Tune 交易窗口与 Fine Tune 设置从高级调优设置移至 Auto Tune（训练/验证/测试）区，与历史年数、测试段同行展示。
...
- [改进] Web 首页顶部搜索栏缩短并与报告操作按钮同行右对齐：重新分析 / 咨询 AI / 历史趋势 / 完整报告按钮移至搜索栏同一行右侧（仅在选中报告时显示）。
- [改进] 摘要股票代码盒宽度自适应代码长度，触发颜色仅作用于盒子背景（浅色 7 天内 / 深色 3 天内触发，绿买红卖）。
- [修复] 修复 Auto Tune 交易窗口与历史年数输入框无法清空重输的问题：输入时不再逐键钳制到最小值，允许完全清空后重新输入，失焦时若为空则恢复上次有效值、非法值钳制到有效范围。
- [改进] Web 首页摘要板块随阈值调整实时刷新：技术指标图保存阈值（手动修改 / 应用 Auto Tune 参数 / 恢复默认）后通过事件通知首页，自选股阈值变化经短暂防抖后自动按新阈值重查综合评分触发并更新摘要颜色，无需刷新页面。
- [改进] 持仓表列间距统一：首页当前持仓表去掉固定列宽改用内容自适应布局，各列间距恢复均匀；组合页持仓表统一各列右侧内边距。
- [新功能] 当前持仓板块支持折叠/展开：标题栏新增折叠图标按钮，可收起持仓表与收益走势图，折叠状态保存在浏览器本地，刷新后保持。
- [新功能] 当前持仓表每行新增卖出按钮：点击后填写卖出日期、卖出价和卖出数量并自动计算本次盈亏（卖出价-买入价）×数量；支持部分卖出（剩余数量保留在原批次），全部卖出后该行移除；卖出记录保存在浏览器本地并汇总已实现总盈亏；下方收益图新增右侧第二轴，以虚线展示已实现盈亏累计曲线。
- [新功能] 收益图下方新增可折叠交易历史表：展示全部卖出记录（买入日期/买入价、卖出日期/卖出价、数量、盈亏），支持编辑（自动重算盈亏）、删除（仅移除记录不影响持仓）和撤销（把卖出数量加回原持仓批次）；折叠状态保存在浏览器本地。
...
- [修复] 技术指标图 SMA200 均线不再只显示在图表尾部：`GET /api/v1/stocks/{code}/indicators` 按可见区间额外多取 200 天历史作为预热窗口，使 SMA200 等长窗口指标覆盖整个可见区间。
- [新功能] 主页报告区顶部新增内联技术指标图（非弹窗），复刻 Excel "300 Plot" 逻辑：收盘价线、SMA5/10/20/30/40/60/120 虚线与 SMA200 红线，叠加 MACD/OBV/KDJ/RSI 买卖触发标记；新增 `GET /api/v1/stocks/{code}/indicators` 后端接口按 Excel 公式计算 SMA/EMA/MACD/KDJ/RSI/BOLL/CCI/OBV，支持可编辑触发阈值（BOL 常数、MACD/KDJ/RSI 买卖阈值）与可选历史区间（30/60/90/180/365 天）。
- [改进] 主页内联技术指标图改为随报告默认自动展示（移除"股价走势"切换按钮），并随容器宽度自适应：用 ResizeObserver 测量容器宽度后按 1:1 渲染，避免宽窗口下线条被放大；大区间数据仍保留横向滚动。
- [改进] 全局布局改为满宽显示：左侧垂直导航栏贴靠窗口最左边缘，页面主内容按窗口宽度自适应，移除原来的 `max-width` 居中约束。
- [改进] 主页内联技术指标图默认以 30 天区间打开（原默认 365 天）。
- [改进] 技术指标图最新价数字始终渲染（加载中显示 `--`），不因数据未就绪而隐藏，避免 365 天视图下价格数字缺失。
- [改进] 技术指标图最新价数字改为白色（`text-foreground`），并在"历史/自选/今日"各标签页的 365 天视图下均正常显示。
- [改进] 技术指标图新增 X 轴日期刻度（各区间标签页均显示 5 个日期刻度）。
- [改进] 技术指标图按容器宽度自适应铺满（365 天等大区间不再出现横向滚动条）。
- [改进] 技术指标图新增 MACD / OBV / KDJ 触发标记显示开关，切换时实时更新图表并同步图例状态。
- [修复] 报告"核心洞察"段落改为 `break-words`，长内容（URL/长数字等）可正确换行。
- [改进] 技术指标图图例补齐 SMA5/10/20/30/40/60/120 各条均线图例（与 SMA200 一致）。
- [新功能] 技术指标触发阈值按股票分别记忆（localStorage，键 `dsa.indicator.thresholds.<代码>`）：切换股票或刷新后自动恢复各股自定义阈值。
- [改进] 侧边工作区"自选"与"历史"标签位置互换（自选在前，历史居中）。
- [改进] 侧边工作区"自选"列表选中行高亮样式改为与"历史"一致（复用 `home-history-item-selected` 的青色渐变描边、渐变背景与发光阴影）。
- [新功能] 新增 Windows 一键本地启动脚本，自动检查/启动 Ollama、确保本地模型可用，并启动 FastAPI 服务。
- [改进] Windows 一键本地启动脚本在 FastAPI 服务就绪后自动用默认浏览器打开 WebUI（`http://127.0.0.1:8000`），服务仍在当前窗口前台运行，Ctrl+C 可停止。
