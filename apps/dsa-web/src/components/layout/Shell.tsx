...
export const Shell: React.FC<ShellProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="flex min-h-screen w-full py-3 sm:py-4">
        <aside
          className={cn(
            'sticky top-3 z-40 hidden shrink-0 overflow-visible rounded-r-[1.5rem] border border-l-0 border-[var(--shell-sidebar-border)] bg-card/72 p-2.5 shadow-soft-card backdrop-blur-sm transition-[width] duration-200 lg:flex',
        ...
        <main className="min-h-0 min-w-0 flex-1 px-3 pt-14 sm:px-4 lg:pl-4 lg:pr-5 lg:pt-0 touch-pan-y">
          {children ?? <Outlet />}
        </main>
      </div>
