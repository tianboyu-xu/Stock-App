...
export interface UseWatchlistReturn {
  ...
  reorderWatchlist: (orderedCodes: string[]) => Promise<void>;
...
export function useWatchlist(): UseWatchlistReturn {
  ...
  const reorderWatchlist = useCallback(async (orderedCodes: string[]) => {
    if (isActioning) return;
    const previousCodes = codes;
    setIsActioning(true);
    if (mountedRef.current) {
      setCodes(orderedCodes);
    }
    try {
      const result = await systemConfigApi.reorderWatchlist(orderedCodes);
      if (mountedRef.current) {
        setCodes(result);
      }
    } catch {
