...
describe('HomePage', () => {
  beforeEach(() => {
    ...
    vi.mocked(stocksApi.getHistory).mockResolvedValue({
      ...
      period: 'daily',
      data: [
        { date: '2026-03-01', open: 100, high: 105, low: 99, close: 104, volume: 1000, changePercent: 2 },
        { date: '2026-03-02', open: 104, high: 108, low: 103, close: 107, volume: 1200, changePercent: 2.88 },
      ],
    });
  ...
  it('shows the inline indicator chart by default and fetches configurable history lengths', async () => {
    vi.mocked(historyApi.getList).mockResolvedValue({
      total: 1,
      page: 1,
      limit: 20,
    ...
    // The report auto-loads and the inline indicator chart shows without any toggle button.
    expect(await screen.findByText('技术指标')).toBeInTheDocument();
    await waitFor(() => {
      expect(vi.mocked(stocksApi.getIndicators)).toHaveBeenCalledWith('600519', { period: 'daily', days: 180 });
    });
    expect(screen.getAllByText('贵州茅台 · 600519').length).toBeGreaterThan(0);
    // Wait for the async indicator fetch to resolve and the chart to render
    expect(await screen.findByRole('img', { name: '技术指标' })).toBeInTheDocument();
    expect(
      await screen.findByText((content) => content.includes('2026-03-01 ~ 2026-03-02')),
    ).toBeInTheDocument();

    // Switch to a different history length
    ...
    await waitFor(() => {
      ...
      expect(lastCall[1]).toMatchObject({ period: 'daily', days: 90 });
    });
  });
  ...
  it('loads the S&P 500 index chart when the auto-selected report is a market review', async () => {
    vi.mocked(historyApi.getList).mockResolvedValue({
      total: 1,
      page: 1,
      limit: 20,
    ...
    expect(await screen.findByText('大盘复盘摘要')).toBeInTheDocument();
    // A market-review report uses the synthetic "MARKET" code; the chart should
    // request the S&P 500 index (SPX) instead of failing on "MARKET".
    await waitFor(() => {
      const calls = vi.mocked(stocksApi.getIndicators).mock.calls;
      expect(calls.length).toBeGreaterThan(0);
      expect(calls[0][0]).toBe('SPX');
    });
    expect(vi.mocked(stocksApi.getIndicators).mock.calls[0][0]).not.toBe('MARKET');
  });
