...
import { StockHistoryTrendDrawer, StockIndicatorChart } from '../components/history';
...
const HomePage: React.FC = () => {
  ...
  const [priceHistoryCode, setPriceHistoryCode] = useState<string | null>(null);
  ...
  // Keep the inline indicator chart synced to the currently selected report's stock.
  // A market-review report uses the synthetic code "MARKET", which is not a real
  // symbol; map it to the S&P 500 index (SPX) so the price chart can load real data.
  const priceHistoryCodeKey = isHistoryTrendUnavailable
    ? ''
    : isMarketReviewHistoryReport
      ? 'SPX'
      : selectedReport.meta.stockCode;
  useEffect(() => {
    setPriceHistoryCode(priceHistoryCodeKey || null);
  }, [priceHistoryCodeKey]);
  ...
  const sidebarContent = useMemo(
    () => (
      <div className="flex h-full min-h-0 flex-col gap-2 overflow-hidden">
        ...
        <HomeStockWorkspace
          ...
          onReorderWatchlist={watchlistState.reorderWatchlist}
    ...
    [
      ...
      watchlistState.reorderWatchlist,
    ],
  );
    ...
    >
      <div className="flex-1 flex flex-col min-h-0 min-w-0 w-full">
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          <div className="hidden min-h-0 w-64 shrink-0 flex-col overflow-hidden pb-4 md:flex lg:w-72">
            {sidebarContent}
          </div>
