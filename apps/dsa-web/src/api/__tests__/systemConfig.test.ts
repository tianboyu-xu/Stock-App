...
describe('systemConfigApi', () => {
  ...
  it('reorders the watchlist via the reorder endpoint', async () => {
    post.mockResolvedValueOnce({
      data: {
        stock_codes: ['AAPL', '600519', '000001'],
        message: '已重排自选队列，共 3 只股票',
      },
    });

    const result = await systemConfigApi.reorderWatchlist(['AAPL', '600519', '000001']);

    expect(post).toHaveBeenCalledWith('/api/v1/stocks/watchlist/reorder', {
      stock_codes: ['AAPL', '600519', '000001'],
    });
    expect(result).toEqual(['AAPL', '600519', '000001']);
  });
});
