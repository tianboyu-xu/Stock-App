...
from api.v1.schemas.history import WatchlistReorderRequest, WatchlistRequest
...
def test_watchlist_reorder_applies_requested_order() -> None:
    service = FakeSystemConfigService("600519,300750,AAPL")

    response = reorder_watchlist(
        WatchlistReorderRequest(stock_codes=["AAPL", "600519", "300750"]),
        service=service,
    )

    assert response.stock_codes == ["AAPL", "600519", "300750"]
    assert service.stock_list == "AAPL,600519,300750"
    assert service.update_calls == ["AAPL,600519,300750"]


def test_watchlist_reorder_ignores_unknown_and_duplicate_codes() -> None:
    service = FakeSystemConfigService("600519,300750,AAPL")

    response = reorder_watchlist(
        WatchlistReorderRequest(stock_codes=["AAPL", "AAPL", "999999", "600519"]),
        service=service,
    )

    # Unknown 999999 dropped, duplicate AAPL deduped, missing 300750 appended.
    assert response.stock_codes == ["AAPL", "600519", "300750"]
    assert service.stock_list == "AAPL,600519,300750"


def test_watchlist_reorder_with_empty_request_keeps_existing_order() -> None:
    service = FakeSystemConfigService("600519,300750,AAPL")

    response = reorder_watchlist(
        WatchlistReorderRequest(stock_codes=[]),
        service=service,
    )
